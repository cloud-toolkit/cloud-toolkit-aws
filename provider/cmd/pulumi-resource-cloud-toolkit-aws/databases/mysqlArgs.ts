...
export function validateConfig(c: MysqlArgs): MysqlArgs {
  ...
  if (
    ...
    (Object.keys(MysqlVersionArgs).includes(config.version) === false &&
      Object.values(MysqlVersionArgs).includes(config.version) === false)
  ) {
    throw Error(
      "You must select one allowed engine version for Database: " +
        Object.keys(MysqlVersionArgs)
    );
  }
